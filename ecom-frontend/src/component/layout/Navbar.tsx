...
function AccountMenu() {
  ...
  async function handleLogout() {
    try {
      ...
      addToast('Logout failed on server, but local session was cleared', 'warning')
    } finally {
      setOpen(false)
      navigate('/')
    }
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-52 bg-white rounded-2xl shadow-xl border border-stone-100 py-2 z-50 animate-in">
          ...
          {isAdmin && (
            <>
              <Link
                ...
                className="flex items-center gap-2.5 px-4 py-1.5 text-sm font-semibold text-violet-700 hover:bg-violet-50 transition-colors"
              >
          ...
          <Link
            to="/profile"
            onClick={() => setOpen(false)}
            className="flex items-center gap-2.5 px-4 py-1.5 text-sm text-stone-700 hover:bg-stone-50 transition-colors"
          >
          ...
          <button
            ...
            className="w-full flex items-center gap-2.5 px-4 py-1.5 text-sm text-red-500 hover:bg-red-50 transition-colors"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2.2} viewBox="0 0 24 24">
